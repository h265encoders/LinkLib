...
public:
    ...
    struct Packet{
        ...
        qint64 dts;
        enum  PacketType{
            IFrame, PFrame, BFrame, AFrame, VFrame , SPFrame, SPFrame_info
        }type;
        LinkObject *sender;
    ...
    struct StreamInfo{
        enum DataType{
            UnKnow, VPSS, Raw, FLTP, H264, H265, MJPEG, PCMA, PCMU, G722, AAC, MP2, MPEGTS, RTSP, MPEG4, MP3, AC3, OPUS
        }type;
        QVariantMap info;
        LinkObject *sender;
    ...
    struct Buffer{
        ...
        QList<Packet> buffer;
        int len;
        Buffer(): lock(QReadWriteLock::Recursive){
    ...
    LinkObject* unLinkE(LinkObject* dst);
    ...
    StreamInfo getStreamInfoA();
    StreamInfo getStreamInfoV();
protected:
...
public slots:
    ...
    void onUnLinkAPriv(StreamInfo info);
    void onUnLinkVPriv(StreamInfo info);
    virtual void onNewEvent(QString type,QVariant info);
private slots:
